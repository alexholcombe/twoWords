...
#letter size 2.5 deg
SOAms = 100 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133   #KR: was 233
...
letterDurMs =   80 #Was 17. 23.6  in Martini E2 and E1b (actually he used 22.2 but that's because he had a crazy refresh rate of 90 Hz = 0
...
if doStaircase:
    ...
    while nDoneMain < trials.nTotal and expStop==False: #MAIN EXPERIMENT LOOP
        NextRemindCountText.setText('BEGINNING OF TRIAL LOOP');        NextRemindCountText.draw(); myWin.flip() #speedup
        if nDoneMain==0:
        ...
        NextRemindCountText.setText('ABOUT TO PREDRAW STIMULI');        NextRemindCountText.draw(); myWin.flip() #speedup
        sequenceStream1, sequenceStream2 = calcAndPredrawStimuli(wordList1,wordList2)
        NextRemindCountText.setText('AFTER CALC AND PREDRAW STIMULI');        NextRemindCountText.draw(); myWin.flip() #speedup
        ...
        if not expStop:
                ...
                NextRemindCountText.setText('END OF TRIAL'); NextRemindCountText.draw(); myWin.flip() #speedup
