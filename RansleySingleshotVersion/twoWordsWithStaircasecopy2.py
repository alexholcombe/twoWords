...
#####shuffles wordlist for each participant.
    ...
    #seq1 is an array of indices corresponding to the appropriate pre-drawn stimulus, contained in textStimuli
      ...
      cueFrames = 0 #  thisTrial['cuePos']*SOAframes  #cuesPos is global variable
...
#      textStimuliStream2[thisStim2Idx].flipHoriz = thisTrial['rightStreamFlip']
    ...
    noiseFieldWidthDeg=ltrHeight * 10.0
    ...
    def handleAndScoreResponse(passThisTrial,response,task,stimSequence,cuePos,correctAnswerIdx,wordList):
        ...
        correctAnswer = wordList[staircaseTrialN].upper()
    ...
    if doStaircase:
        ...
        while (not staircase.finished) and not expStop: #staircase.thisTrialN < staircase.nTrials
            if staircaseTrialN+1 < len(prefaceStaircaseNoise): #still doing easyStaircaseStarterNoise
                staircaseTrialN += 1
                print(staircaseTrialN)
                print(len(prefaceStaircaseNoise))
                ...
                if staircaseTrialN+1 == len(prefaceStaircaseNoise): #add these non-staircase trials so QUEST knows about them
                    ...
                    print('not Importing (commented out)',corrEachTrial,' and intensities ',prefaceStaircaseNoise)
            ...
            #thisStaircaseTrial = staircaseTrialN.next()
            #thisProbe = thisStaircaseTrial['probe']
            #if thisProbe=='both':
            #  numRespsWanted = 1
            ...
            Target = TargetList[staircaseTrialN] 
            numRespsWanted=1
            ...
            for RespProbeFrame in xrange(RespProbeDurFrames):
                if (Target=='R'): #change order of indices depending on LeftTarget. response0, answer0 etc refer to which one had to be reported first
                    ...
                    myWin.flip()
                    answer0 = wordList2[ staircaseTrialN ]
                    answer1 = wordList2[ staircaseTrialN ]
                    ...
                    RespProbeBottom.draw()
                    myWin.flip() 
                    answer0 = wordList1[ staircaseTrialN ]
                    answer1 = wordList1[ staircaseTrialN ]
...
#                    play_high_tone_correct_low_incorrect(correct, passThisTrial=False)
                print('staircaseTrialN=', staircaseTrialN,' noisePercent=',round(noisePercent,3),' approxCorrect=',trialCorrect) #debugON
                #corrEachTrial.append(correct)
                if mainStaircaseGoing: 
                    if Target == 'R':
                        targetR +=1
                        if trialCorrect==True and targetR<2: #so a second right hand target 
                            numCorrect +=1
                    else:
                        targetL +=1
                        if trialCorrect==True and targetL<2:
                            numCorrect +=1
                        
                    if targetL>0 and targetR>0:
                        if numCorrect>0:
                            correct=True
                        else:
                            correct=False
                        staircase.addResponse(correct, intensity = 100-noisePercent) #Add a 1 or 0 to signify a correct/detected or incorrect/missed trial                        
                        print('Have added an intensity of','{:.3f}'.format(100-noisePercent), 'correct =', correct, ' to staircase') #debugON
                        totalTrials+=1
                        targetR=0
                        targetL=0
                        numCorrect=0
        ...
        if staircaseTrialN+1 < len(prefaceStaircaseNoise) and (staircaseTrialN>=0): #exp stopped before got through staircase preface trials, so haven't imported yet
            print('Importing (not, because commented out) ',corrEachTrial,' and intensities ',prefaceStaircaseNoise[0:staircaseTrialN+1])
        ...
        noisePercent = 50 #defaultNoiseLevel
        ...
        while nDoneMain < trials.nTotal and not expStop: #MAIN EXPERIMENT LOOP while nDoneMain < len(wordList1) and not expStop:
            ...
            cuesPos,correctAnswerIdxsStream1,correctAnswerIdxsStream2, ts  = \
                                                                        do_RSVP_stim(thisTrial, idxsStream1, idxsStream2, noisePercent/100.,nDoneMain)
