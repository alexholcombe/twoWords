...
SOAms = 200 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133
...
def calcAndPredrawStimuli(thisTrial,wordList): #called before each trial
    ...
    whichList = np.random.randint(0, 2)
   ...
   for rightResponseFirst in [False,True]:
      for bothWordsFlipped in [True]:
        for orthographicalsim in [True,False]:
          stimList.append( {'cuePos':cuePos, 'rightResponseFirst':rightResponseFirst, 'flipHoriz':bothWordsFlipped, 'flipVert':True,
                                    'orthographicalsim':orthographicalsim} ) # add 'orthographicalsim':orthographical similarity
...
print('noisePercent\tflipHoriz\tflipVert\torthographicalsim\t',end='',file=dataFile)#added tabs for the two flips
if task=='T1':
    numRespsWanted = 2
...
#seq1 is an array of indices corresponding to the appropriate pre-drawn stimulus, contained in textStimuli
  ...
  textStimuliStream1[thisStimIdx].flipVert = thisTrial['flipVert'] #added this and the next line in to try to invert
  textStimuliStream2[thisStim2Idx].flipVert = thisTrial['flipVert']
  textStimuliStream1[thisStimIdx].flipHoriz = thisTrial['flipHoriz']
  textStimuliStream2[thisStim2Idx].flipHoriz = thisTrial['flipHoriz']
