...
numWordsInStream = 24
wordsUnparsed="the, and, for, you, say, but, his, xxx, xxx, xxx, xxx, xxx, xxx, xxx, xxx, xxx, xxx, xxx, now, how, its, our, two, way" #24 most common words
...
def calcAndPredrawStimuli(thisTrial,wordList): #called before each trial
    ...
    orthographicallySimilarList = ['bed','now','ban']
    orthographicallyDissimilarList = ['off','egg','her']
    if thisTrial['orthographicalsim'] == 'Yes':
        listForThisCondition = orthographicallySimilarList
    elif  thisTrial['orthographicalsim'] == 'No':
        listForThisCondition = orthographicallyDissimilarList
    ...
    for word in listForThisConditionLeft:
        ...
        wordList.append(word) #add new words to end of wordList
    ...
    print('numWordsToLoadUp=',numWordsToLoadUp, ' length of wordList=',len(wordList))
    for i in range(0,numWordsToLoadUp): #draw the words that will be used on this trial, the first 26 of the shuffled list
       word = wordList[ i ]  #     #[ idxsIntoWordList[i] ] the below is the way that Psychopy draws texton screen
       ...
       print('Kiuyan modified wordList ',i,' =',word)
    ...
    for i in range(8,17): #ends up adding 10??
        idxOfWordToSubstituteLeft =  numWordsInStream+(i-8)
        idxsStream1[i] = idxOfWordToSubstituteLeft
        idxOfWordToSubstituteRight =  numWordsInStream + toSubstituteOnRight[i-8]
        idxsStream2[i] = idxOfWordToSubstituteRight
...
for cuePos in cuePositions:
   for rightResponseFirst in [False,True]:
      for bothWordsFlipped in [True]:
        for orthographicalsim in ['Yes', 'No']: #need to make sure can embed in 
...
print('noisePercent\tleftStreamFlip\trightStreamFlip\tflipHoriz\tflipVert\t',end='',file=dataFile)#added tabs for the two flips
if task=='T1':
    numRespsWanted = 2
