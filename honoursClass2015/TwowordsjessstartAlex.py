...
numWordsInStream = 24
listOfWordLists =[["the, and, for, you, say, but, his, not, she, can, who, get, her, all, one, out, see, him, now, how, its, our, two, way"],
["the, and, for, you, say, but, his, not, she, can, who, get, her, all, one, out, see, him, now, how, its, our, two, way"],
["the, and, for, you, say, but, his, not, she, can, who, get, her, all, one, out, see, him, now, how, its, our, two, way"]]

wordsUnparsed="away, ache, best, body, city, deaf, disk, dome, fair, flow, halo, heir, less, melt, mild, numb, part, pier, quay, rare, rote, seen, size, tape"
nonwordsUnparsed="hin, hep, abc, def, ghi, jkl, mno, pqr, stu, vwx, yza, bcd, efg, hij, klm, nop, qrt, vwe, tml, ghj, asdf, mnp, jgh, twl" 
...
if len(wordList) != numWordsInStream:
    print('WARNING: You provided a wordList that is of length ',len(wordList),' but I was expecting ',numWordsInStream)
nonwordList = nonwordsUnparsed.split(",") #split into list
if len(nonwordList) != numWordsInStream:
    print('WARNING: You provided a nonwordList that is of length ',len(nonwordList),' but I was expecting ',numWordsInStream)
for i in range(len(wordList)):
    wordList[i] = wordList[i].replace(" ", "") #delete spaces
for i in range(len(nonwordList)):
    nonwordList[i] = nonwordList[i].replace(" ", "") #delete spaces
...
#letter size 2.5 deg
SOAms = 250 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133 #use 133
...
letterDurMs = 200 #23.6  in Martini E2 and E1b (actually he used 22.2 but that's because he had a crazy refresh rate of 90 Hz) #80 before
...
def calcAndPredrawStimuli(wordList,nonwordList,wordNonword):
    if len(wordList) < numWordsInStream:
    ...
    idxsIntoNonWordList = np.arange( len(nonwordList) )
    print('wordList=',wordList)
    print('nonwordList=',nonwordList)
    for i in range(0,numWordsInStream): #draw the words that will be used on this trial, the first x of the shuffled list
       if wordNonword:  #TRUE means nonword, FALSE means use words
         word = wordList[ i ]
         nonword = nonwordList [ i ]
       else: #FALSE, therefore use word on both sides
         word = wordList[ i ]  
         nonword = wordList[ i ]  
       #flipHoriz, flipVert  textStim http://www.psychopy.org/api/visual/textstim.html
       #always draw 'word' in the left stream
       ...
       #always draw 'nonwrod' in the right stream
       textStimulusStream2 = visual.TextStim(myWin,text=nonword,height=ltrHeight,colorSpace='rgb',color=letterColor,alignHoriz='center',alignVert='center',units='deg',autoLog=autoLogging)
...
for cuePos in cuePositions:
   for rightResponseFirst in [False,True]:
      for bothWordsFlipped in [False,False]:
        for wordNonword in [True]:
            stimList.append( {'cuePos':cuePos, 'rightResponseFirst':rightResponseFirst,
                                    'leftStreamFlip':bothWordsFlipped, 'rightStreamFlip':bothWordsFlipped, 'wordNonword':wordNonword} )
...
def handleAndScoreResponse(passThisTrial,response,responseAutopilot,task,stimSequence,cuePos,listForScoring,correctAnswerIdx):
    ...
    responseWordIdx = wordToIdx(responseString,listForScoring)
    if responseWordIdx is None: #response is not in the listForScoring
        posOfResponse = -999
    ...
    #print('wordToIdx(',responseString,',',listForScoring,')=',responseWordIdx,' stimSequence=',stimSequence,'\nposOfResponse = ',posOfResponse) #debugON
...
if doStaircase:
    ...
    while (not staircase.finished) and expStop==False: #staircase.thisTrialN < staircase.nTrials
        ...
        wordOrNonword = round( np.random.rand(1) )
        idxsStream1, idxsStream2 = calcAndPredrawStimuli(wordList,nonwordList, wordOrNonword)
        ...
        print("We just scored the first response and wordList is now=",wordList)
    ...
    while nDoneMain < trials.nTotal and expStop==False: #MAIN EXPERIMENT LOOP
        ...
        sequenceStream1, sequenceStream2 = calcAndPredrawStimuli(wordList,nonwordList,thisTrial['wordNonword'])
        cuesPos,correctAnswerIdxsStream1,correctAnswerIdxsStream2, ts  = \
        ...
        if not expStop:
            ...
            for i in range(numRespsWanted): #scored and printed to dataFile in left first, right second order even if collected in different order
                if i==0: #How does it know whether the first response was the left or the right???? 
                    sequenceStream = sequenceStream1
                    correctAnswerIdxs = correctAnswerIdxsStream1
                    listForScoring = copy.deepcopy(wordList)
                else: 
                    sequenceStream = sequenceStream2
                    correctAnswerIdxs = correctAnswerIdxsStream2
                    listForScoring = copy.deepcopy(nonwordList)
                print('About to score response num=',i)
                correct,approxCorrect,responsePosRelative = (
                        handleAndScoreResponse(passThisTrial,responses[i],responsesAutopilot[i],task,sequenceStream,thisTrial['cuePos'],listForScoring,
                                                                        correctAnswerIdxs ) )
                eachCorrect[i] = correct
                eachApproxCorrect[i] = approxCorrect  #Why did Kim do this?
