...
#letter size 2.5 deg
SOAms = 333 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133
...
letterDurMs = 200 #23.6  in Martini E2 and E1b (actually he used 22.2 but that's because he had a crazy refresh rate of 90 Hz)
...
def do_RSVP_stim(thisTrial, seq1, seq2, proportnNoise,trialN):
    ...
    whichStreamItemIsTheCue = seq1[ primePos ] 
    targetIdentity = textStimuliStream1[ seq1[ cuesPos[0] ] ]
    targetIdentity = targetIdentity.text
    print(' Your target text in the left stream =',targetIdentity)
    if targetIdentity == "nail":
        primeIdentity = "nale"
    if targetIdentity == "jail":
        primeIdentity = "jale"
    primeIdentity = 'prime'
    '''if cuePositions = "keep"
        then np.array-1 = "keap"
    if cuePositions = "weep"
        then np.array-1 = "weap"
    if cuePositions = "boat"
        then np.array-1 = "bote"
    if cuePositions = "foam"
        then np.array-1 = "fome"
    if cuePositions = "game"
        then np.array-1 = "gaim"
    if cuePositions = "maim"
        then np.array-1 = "mame"
    if cuePositions = "leaf"
        then np.array-1 = "leef"
    if cuePositions = "beef"
        then np.array-1 = "beaf"
    if cuePositions = "milk"
        then np.array-1 = "milc"
    if cuePositions = "silk"
        then np.array-1 = "silc"
    if cuePositions = "goal"
        then np.array-1 = "gole"
    if cuePositions = "foal"
        then np.array-1 = "fole"
    if cuePositions = "move"
        then np.array-1 = "moov"
    if cuePositions = "loom"
        then np.array-1 = "lume"
    if cuePositions = "toad"
        then np.array-1 = "tode"
    if cuePositions = "mode"
        then np.array-1 = "moad"
    if cuePositions = "home"
        then np.array-1 = "hoam"
    if cuePositions = "dome"
        then np.array-1 = "doam"
    if cuePositions = "call"
        then np.array-1 = "cawl"
    if cuePositions = "gall"
        then np.array-1 = "gawl"
    if cuePositions = "bird"
        then np.array-1 = "berd"
    if cuePositions = "burn"
        then np.array-1 = "bern" 
    '''
    textStimuliStream1[ whichStreamItemIsTheCue ].setText( primeIdentity )
